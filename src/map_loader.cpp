...
void MapLoader::findKNeighbor(int start,int start2,int K,std::vector<int> &dist,std::vector<int> &nodeCount)const
{
    dist.resize(cols * rows, 1e8);
    std::queue<int> open;
    std::vector<bool> closed(cols * rows, false);
    open.push(start);
    closed[start] = true;
    ...
    while (!open.empty())
    {
        ...
        if(dist[curr]>0)nodeCount[dist[curr]-1]+=1;
        for (int i = 0; i < 4; i++)
        {
            int next = curr + moves_offset[i];
            if (next < 0 || next >= cols *rows || my_map[next] || closed[next])
